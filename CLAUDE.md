...
### Code Authorship & Headers (HARD)

- [HARD] Author Header: Every source/code file MUST begin with a boxed author header (template below). Any NEW code file is created with this header by default.
  WHY: Consistent provenance and ownership across the codebase.
- [HARD] Author Attribution — Human Only: The author is `Chun Kang <kurapa@kurapa.com>` ONLY. NEVER include Claude or any AI attribution — not in file headers, and not in git commits (do NOT add `Co-Authored-By: Claude ...` trailers).
  WHY: The project is authored and owned by Chun Kang.
- [HARD] Header Date: Use the file's creation date (its first git-add date) with timezone, e.g. `2026-07-03  (PDT, UTC-07:00)`.

Header template — adapt the comment prefix per language (`#` for py/sql/sh/yaml/Dockerfile/nginx-conf, `//` for ts/tsx/js/jsx, wrap in `/* ... */` for css):

```
# ┌──────────────────────────────────────────────────────────────────────────┐
# │ milvus_station                                                           │
# │ Author  : Chun Kang <kurapa@kurapa.com>                                  │
# │ Created : YYYY-MM-DD  (PDT, UTC-07:00)                                   │
# └──────────────────────────────────────────────────────────────────────────┘
```
